...
// static char *quote_heredoc(char *prompt, t_parser *p)
// {
...
// 	// loop into string backward until detecting <<
// 	// if there is a << before the end of the string, save the next char delimited by spaces and return 1
// 	// if no chars after <<, return 0
...
void	get_more_prompt(t_data *data, t_parser *p)
{
	...
	// if (check_valid_last_heredoc(data->prompt) == 1)
